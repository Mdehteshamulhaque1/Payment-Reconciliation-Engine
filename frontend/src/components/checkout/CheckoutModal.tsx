...
import { CreditCard, Smartphone, Globe, CheckCircle2, Loader2, Shield, Lock, Zap, Building2 } from 'lucide-react'
...
type Tab = 'card' | 'upi' | 'international'

const upiApps = [
  { id: 'gpay', name: 'Google Pay', color: '#4285F4', icon: 'GP' },
  { id: 'phonepe', name: 'PhonePe', color: '#5F259F', icon: 'PP' },
  { id: 'paytm', name: 'Paytm', color: '#00BAF2', icon: 'PT' },
  { id: 'bhim', name: 'BHIM', color: '#097969', icon: 'BH' },
  { id: 'amazonpay', name: 'Amazon Pay', color: '#FF9900', icon: 'AP' },
  { id: 'cred', name: 'CRED', color: '#000000', icon: 'CR' },
]
...
export default function CheckoutModal({ open, onClose, plan, planIcon: PlanIcon }: CheckoutModalProps) {
  ...
  const handlePay = useCallback(async () => {
    ...
    let gateway = 'stripe'
    if (tab === 'upi') {
      gateway = selectedUpi === 'gpay' ? 'stripe' : selectedUpi === 'phonepe' ? 'razorpay' : selectedUpi === 'paytm' ? 'razorpay' : 'upi'
            ...
            <div className="relative">
              {step === 'done' ? (
                ...
                <div className="p-8 text-center">
                  ...
                  >
                    Your <strong className="text-[var(--text)]">{plan}</strong> plan is active. You paid ₹1.00 to verify your payment method.
                  </motion.p>
                ...
                <div className="p-8 text-center">
                  ...
                  <h2 className="text-xl font-bold text-[var(--text)] mb-2" style={{ fontFamily: 'Outfit' }}>Processing Payment</h2>
                  <p className="text-sm text-[var(--muted)]">Please wait while we securely process your ₹1 payment...</p>
                </div>
              ) : (
                /* ----- FORM ----- */
                <>
                    ...
                    <div className="flex rounded-xl border border-[color-mix(in_srgb,var(--accent-cyan)_8%,var(--border))] bg-[color-mix(in_srgb,var(--bg3)_40%,transparent)] p-1 gap-1 mb-6">
                      {([
                        ...
                        <button
                          key={t.id}
                          onClick={() => setTab(t.id)}
                          className={`flex-1 flex items-center justify-center gap-2 rounded-lg py-2 text-xs font-mono uppercase tracking-wide transition-all ${
                            tab === t.id
                  ...
                  <div className="px-6 pb-6 max-h-[50vh] overflow-y-auto">
                    ...
                    {tab === 'upi' && (
                      <motion.div
                        ...
                        className="space-y-4"
                      >
                        <p className="text-[10px] font-mono uppercase tracking-widest text-[var(--muted)] text-center">Choose your UPI app</p>
                        <div className="grid grid-cols-3 gap-3">
                          {upiApps.map((app) => (
                            ...
                            >
                              ...
                              <span className="text-[10px] font-mono text-[var(--text)]">{app.name}</span>
                            </button>
                          ))}
                        </div>
                        <div className="relative">
                        ...
                        <div>
                          <label className="mb-1 block text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--muted)] font-mono">Enter UPI ID</label>
                          <input
                            value={upiId}
                            onChange={(e) => { setUpiId(e.target.value); setSelectedUpi(null) }}
                            placeholder="username@payflow"
                            className="w-full rounded-xl border border-border bg-card px-3 py-2.5 text-sm outline-none focus:border-primary font-mono"
                          />
                        </div>
                      </motion.div>
                    ...
                    <Button
                      onClick={handlePay}
                      disabled={!isValid}
                      className="w-full mt-5 border-0 bg-gradient-to-r from-[var(--accent-cyan)] to-[var(--primary)] text-white font-bold tracking-wide hover:shadow-[0_0_30px_rgba(0,240,255,0.3)] transition-all duration-300"
                      size="lg"
                    >
                      <Lock size={14} className="mr-2" />
                      Pay ₹1.00 — Start Trial
                    </Button>
